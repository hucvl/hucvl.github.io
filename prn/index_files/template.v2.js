...
`);class Di extends Ei(HTMLElement){constructor(){super(),this._numbers=[],this._entries=[]}connectedCallback(){this.outerSpan=this.root.querySelector('#citation-'),this.innerSpan=this.root.querySelector('.citation-number'),this.hoverBox=this.root.querySelector('d-hover-box'),window.customElements.whenDefined('d-hover-box').then(()=>{this.hoverBox.listen(this)}),this.numbers&&this.displayNumbers(this.numbers),this.entries&&this.displayEntries(this.entries)}static get observedAttributes(){return['key']}attributeChangedCallback(e,t,n){const i=t?'onCiteKeyChanged':'onCiteKeyCreated',a=n.split(',').map((e)=>e.trim()),d={detail:[this,a],bubbles:!0},r=new CustomEvent(i,d);document.dispatchEvent(r)}set key(e){this.setAttribute('key',e)}get key(){return this.getAttribute('key')}get keys(){return this.getAttribute('key').split(',')}set numbers(e){this._numbers=e,this.displayNumbers(e)}get numbers(){return this._numbers}displayNumbers(e){if(this.innerSpan){const t=e.map((e)=>{return-1==e?'?':e+1+''}),n='['+t.join(', ')+']';this.innerSpan.textContent=n}}set entries(e){this._entries=e,this.displayEntries(e)}get entries(){return this._entries}displayEntries(e){this.hoverBox&&(this.hoverBox.innerHTML=`<ul>
...
`,Nr=ti('distill-header',Ur,!1);class Ir extends Nr(HTMLElement){}const jr=`
<style>
  distill-appendix {
    contain: layout style;
  }
...
`;class Rr extends HTMLElement{static get is(){return'distill-appendix'}set frontMatter(e){this.innerHTML=Ln(e)}}const qr=`
<style>
...
`,Fr=ti('distill-footer',qr);class Pr extends Fr(HTMLElement){}const Hr=function(){if(1>window.distillRunlevel)throw new Error('Insufficient Runlevel for Distill Template!');if('distillTemplateIsLoading'in window&&window.distillTemplateIsLoading)throw new Error('Runlevel 1: Distill Template is getting loaded more than once, aborting!');else window.distillTemplateIsLoading=!0,console.debug('Runlevel 1: Distill Template has started loading.');p(document),console.debug('Runlevel 1: Static Distill styles have been added.'),console.debug('Runlevel 1->2.'),window.distillRunlevel+=1;for(const[e,t]of Object.entries(hi.listeners))'function'==typeof t?document.addEventListener(e,t):console.error('Runlevel 2: Controller listeners need to be functions!');console.debug('Runlevel 2: We can now listen to controller events.'),console.debug('Runlevel 2->3.'),window.distillRunlevel+=1;if(2>window.distillRunlevel)throw new Error('Insufficient Runlevel for adding custom elements!');const e=[ki,wi,Si,Li,Ai,Di,Oi,Ii,Ri,Fi,pi,Hi,zi,T,Bi,Wi,Vi,Mr,$i].concat([Ir,Rr,Pr]);for(const t of e)console.debug('Runlevel 2: Registering custom element: '+t.is),customElements.define(t.is,t);console.debug('Runlevel 3: Distill Template finished registering custom elements.'),console.debug('Runlevel 3->4.'),window.distillRunlevel+=1,s()&&hi.listeners.DOMContentLoaded(),console.debug('Runlevel 4: Distill Template initialisation complete.')};window.distillRunlevel=0,yi.browserSupportsAllFeatures()?(console.debug('Runlevel 0: No need for polyfills.'),console.debug('Runlevel 0->1.'),window.distillRunlevel+=1,Hr()):(console.debug('Runlevel 0: Distill Template is loading polyfills.'),yi.load(Hr))});
